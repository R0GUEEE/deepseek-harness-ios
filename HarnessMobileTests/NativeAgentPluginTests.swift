...
final class NativeAgentPluginTests: XCTestCase {
    func testNativeCompilationDiagnosticRoundTripsAndKeepsRetryContract() throws {
        let diagnostic = NativeAgentCompilationDiagnostic(
            ...
            suggestedAction: "Retry with the same token after fixing"
        )
    ...
    func testSkillRadarManifestWithLocalizedToolGuardMaterializes() throws {
        let manifest = Data("""
        {
          ...
          "tool_guards": [{"label": "skill_radar read-only scan", "tool_names": ["skill_radar"], "risks": ["pure"], "decision": "allow", "reason": "Read-only scan: no network, no writes, no external commands."}],
        ...
        XCTAssertEqual(plugin.toolGuards.first?.label, "skill_radar read-only scan")
    ...
    func testListGatedHiddenPathReadManifestIsRejectedWithRepairGuidance() throws {
        ...
        XCTAssertThrowsError(
            ...
            XCTAssertTrue(error.localizedDescription.contains("Direct read"))
    ...
    func testNativeToolGuardAcceptsHumanReadableLocalizedLabel() throws {
        let plugin = try makePlugin(
            toolGuards: [
                NativeAgentToolGuard(
                    label: "skill_radar read-only scan",
        ...
        let plugin = try makePlugin(
            toolGuards: [
                NativeAgentToolGuard(
                    ...
                    risks: [.pure],
                    decision: .allow,
                    reason: "Read-only: enumerate the skills visible to the session and score local relevance."
                )
            ]
        )
    ...
    func testNativeToolGuardReportsTheInvalidField() {
        XCTAssertThrowsError(
            ...
            XCTAssertEqual(
                error.localizedDescription,
                "The native plugin generated by the on-device Agent failed validation: invalid tool_guards[0]: provide at least one of tool_names or risks."
            )
        }
    }
