    ...
    private func trajectoryLedger(_ projection: TrajectoryProjection) -> some View {
        if projection.isEmpty(for: state.mode) {
            ContentUnavailableView(
                ...
                description: Text(
                    state.query.isEmpty
                        ? "After a session runs, requests, messages and on-device tool events appear here."
                        : "Try searching an event type, model, tool name, or call ID."
                )
            )
...
private struct TrajectoryMetricsHeader: View {
    ...
    var body: some View {
            ...
            ViewThatFits(in: .horizontal) {
                ...
                VStack(spacing: 8) {
                    HStack(spacing: 0) {
                        ...
                        TrajectoryPrimaryMetric(title: "Turn", value: String(summary.turns))
                    }
                    HStack(spacing: 0) {
                        TrajectoryPrimaryMetric(title: "Call", value: String(summary.calls))
                    }
                }
            }
        ...
        .accessibilityLabel("Trajectory stats")
    }
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            LazyHStack(spacing: 8) {
                ForEach(entries) { entry in
                    ...
                    .accessibilityHint("Jump to this turn")
                }
            }
            .padding(.horizontal, 14)
            .padding(.vertical, 8)
...
private struct TrajectoryCallSection: Identifiable {
    ...
    var detail: String {
        let status = isError ? "Error" : events.contains { $0.toolResultData != nil } ? "Done" : "Running"
...
private struct TrajectorySectionHeader: View {
    ...
    var body: some View {
        ...
        .accessibilityHint(isCollapsed ? "Show event" : "Collapse event")
    }
}
    ...
    init(event: SessionEvent) {
        ...
        case SessionEventVocabulary.questionRequested:
            ...
            if let request = event.questionRequestedData {
                ...
                subtitle = String(request.questionCount) + " questions · waiting for an answer"
            } else {
        ...
        case SessionEventVocabulary.turnEnd:
            kind = "Turn"
            title = "Turn " + String(event.turnEndData?.turn ?? 0) + " finished"
        ...
        case SessionEventVocabulary.stepStart, SessionEventVocabulary.stepEnd:
            ...
            subtitle = event.type == SessionEventVocabulary.stepStart ? "Start" : "Finished"
...
private struct HarnessTraceInspectorView: View {
    ...
    var body: some View {
        NavigationStack {
            List {
                if let summary {
                    Section {
                        ...
                        LabeledContent("Turn", value: String(summary.turns))
                        LabeledContent("Call", value: String(summary.calls))
                        LabeledContent(
                            "Time to first token",
            ...
            .navigationBarTitleDisplayMode(.inline)
            .searchable(text: $query, prompt: "Search checkpoint, plugin or handler")
            .toolbar {
...
private struct HarnessTracePresentation {
    ...
    init(event: HarnessTraceEvent) {
        ...
        case .checkpointFinished:
            kind = "Checkpoint"
            subtitle = handlers.isEmpty ? "Done" : handlers.joined(separator: " → ")
...
private struct HarnessTraceEventInspectorView: View {
    ...
    var body: some View {
        ...
        NavigationStack {
            List {
                Section {
                    LabeledContent("Index", value: String(event.sequence))
        ...
        .task(id: event.id) {
            ...
            formattedEvent = await Task.detached(priority: .userInitiated) {
                ...
                guard let data = try? encoder.encode(event) else { return "Could not encode the event." }
...
private struct TrajectoryEventInspectorView: View {
    ...
    var body: some View {
        ...
        NavigationStack {
            List {
                Section {
                    LabeledContent("Index", value: String(event.seq))
    ...
    private var typedDetailSections: some View {
        if let header = event.requestHeaderData {
            Section {
                ...
                Label("Request headers", systemImage: "arrow.up.doc")
            }
        }
